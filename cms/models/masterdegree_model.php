...
class Masterdegree_Model extends Model {
    public $_menu;
    ...
    public function listItems($select = false) {
        ...
        $query = "SELECT P.`id`, P.`parent`, T.`title`, T.`specialization`, T.`text`, T.`slug`, P.`ordering`, P.`status`
    ...
    public function createItem() {
        ...
        foreach (MFAdmin::$_langs as $key=>$value){
             ...
             */
            $data['data_specialization_'.$key] = isset($_POST['data_specialization_'.$key]) ? Func::check($_POST['data_specialization_'.$key]) : '';
            ...
            if ($result) {
                ...
                foreach (MFAdmin::$_langs as $key=>$value){
                    ...
                    $insertData = [
                            ...
                            'specialization'=>$data['data_specialization_'.$key],
    ...
    public function singleItem($id) {
        ...
        $query = "SELECT P.`id`, P.`parent`, P.`code`, P.`price`, P.`price_foreign`, P.`post_in_page`, 
                          T.`specialization`, T.`title`, T.`slug`, T.`type`, T.`text`, T.`lang`, P.`ordering`, P.`status` 
                    FROM `cms_undergraduate` P
        ...
        foreach ($resultText as $value){
            ...
            $mas['specialization'][$value['lang']] = $value['specialization'];
