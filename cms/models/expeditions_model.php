...
class Expeditions_Model extends Model {
    ...
    public function listItems() {
        ...
        $query = "SELECT P.`id`, T.`title`, T.`text`, P.`ordering`, P.`update_date`, P.`status`
    ...
    public function deleteItem($items) {
        ...
        $result = $this->db->update('`cms_expeditions`', $updateData, '`id` IN (' . $ids . ')');
    ...
    public function orderingItem($items) {
        foreach ($items as $key => $value) {
            ...
            $result = $this->db->update('`cms_expeditions`', $updateData, '`id` IN (' . $key . ')');
        }
    ...
    public function createItem() {
        ...
        foreach (MFAdmin::$_langs as $key=>$value){
             ...
             * CHECK date
             */
            $data['data_date_'.$key] = isset($_POST['data_date_'.$key]) ? Func::check($_POST['data_date_'.$key]) : '';
        ...
        if (empty($error)){
            ...
            $result = $this->db->insert('cms_expeditions', $insertData);
            ...
            if ($result) {
                ...
                $this->db->update('`cms_expeditions_photo`', $updateData, "`secret_id`='{$data['sid']}'");
                ...
                foreach (MFAdmin::$_langs as $key=>$value){
                    ...
                    $insertData = [
                            ...
                            'exp_date'=>$data['data_date_'.$key],
                            'text'=>$data['data_text_'.$key],
                            'lang'=>$key
                    ];
                    $this->db->insert('cms_expeditions_text', $insertData);
                }
    ...
    public function singleItem($id) {
         ...
         * SELECT `data`
         */
        $query = "SELECT S.`id`, S.`ordering`, S.`status`, T.`title`, T.`exp_date`, T.`text`, T.`lang`, 
	                  (SELECT `secret_id` FROM `cms_expeditions_photo` WHERE p_id = S.id LIMIT 1) as  `secret_id` 
                    FROM `cms_expeditions` S 
                    LEFT JOIN `cms_expeditions_text` T ON T.p_id = S.id   
                    WHERE S.`id`=:id 
                    ORDER BY `ordering`";
        ...
        foreach ($resultText as $value){
            ...
            $mas['exp_date'][$value['lang']] = $value['exp_date'];
    ...
    public function updateItem($id) {
        ...
        if(empty($error)) {
            ...
            $result = $this->db->update('`cms_expeditions`', $updateData, "`id`={$id}");
            ...
            if (isset($result['mysql_error'])) {
                ...
                $deleted = $this->db->delete('cms_expeditions_text', "`p_id`='{$id}'");
                if($deleted) {
                    foreach (MFAdmin::$_langs as $key=>$value){
                        // insert
                        $insertData = [
                            ...
                            if (in_array($file['type'], $allowedContentType)) {
                                if (in_array($imageinfo['mime'], $allowedContentType)) {
                                    ...
                                    if ($handle->uploaded) {
                                        ...
                                        $handle->image_x              = photo_expeditions_width;
                                        $handle->image_ratio_y        = photo_expeditions_height;
                                        ...
                                        $handle->image_x            = thumb_expeditions_width;
                                        $handle->image_y            = thumb_expeditions_height;
                                        $handle->dir_chmod          = 0777;
                                        $handle->process(UPLOAD_DIR . 'Image/'.$this->_menu);
                                        if ($handle->processed) {
                                            $handle->clean();
                                    ...
                                    $result = $this->db->insert('`cms_expeditions_photo`', $insertData);
                                    if($result){
                                        $succeeded[] = array(
                                                'id'=>$result,
                                                'name'=>$file['name'],
    ...
    public function deletePhoto($gid=NULL, $sid=NULL){
        if(isset($gid) && isset($sid)){
            ...
            $query = "SELECT `thumb`, `photo` FROM `cms_expeditions_photo` WHERE `id`=:id and `secret_id`=:secret_id LIMIT 1";
            $deleteDbWhere = "`id`={$gid} and `secret_id`='{$sid}'";
        } else {
            $selectData = [
                    'p_id'=>0
            ];
            $query = "SELECT `thumb`, `photo` FROM `cms_expeditions_photo` WHERE `p_id`=:p_id and date(`date`) < CURDATE()";
        ...
        $this->db->delete('`cms_expeditions_photo`', $deleteDbWhere);
    ...
    public function orderphoto(){
        ...
        if(is_array($updateRecordsArray) && !empty($updateRecordsArray)){
            ...
            foreach ($updateRecordsArray as $recordIDValue) {
                ...
                $this->db->update('`cms_expeditions_photo`', $updateData, "`id`={$id}");
