...
class Gallery_Model extends Model {
    ...
    public function deleteItem($items) {
        ...
        $result = $this->db->update('`cms_gallery`', $updateData, '`id` IN (' . $ids . ')');
    ...
    public function orderingItem($items) {
        foreach ($items as $key => $value) {
            ...
            $result = $this->db->update('`cms_gallery`', $updateData, '`id` IN (' . $key . ')');
        }
    ...
    public function createItem() {
        ...
        if (empty($error)){
            ...
            $result = $this->db->insert('cms_gallery', $insertData);
            ...
            if ($result) {
                ...
                $this->db->update('`cms_gallery_photo`', $updateData, "`secret_id`='{$data['sid']}'");
                ...
                foreach (MFAdmin::$_langs as $key=>$value){
                    ...
                    $this->db->insert('cms_gallery_text', $insertData);
                }
    ...
    public function singleItem($id) {
         ...
         * SELECT `data`
         */
        $query = "SELECT S.`id`, S.`ordering`, S.`status`, T.`title`, T.`lang`, 
	                  (SELECT `secret_id` FROM `cms_gallery_photo` WHERE p_id = S.id LIMIT 1) as  `secret_id` 
                    FROM `cms_gallery` S 
                    LEFT JOIN `cms_gallery_text` T ON T.p_id = S.id   
                    WHERE S.`id`=:id 
                    ORDER BY `ordering`";
    ...
    public function updateItem($id) {
        ...
        if(empty($error)) {
            ...
            $result = $this->db->update('`cms_gallery`', $updateData, "`id`={$id}");
            ...
            if (isset($result['mysql_error'])) {
                ...
                $deleted = $this->db->delete('cms_gallery_text', "`p_id`='{$id}'");
                if($deleted) {
                    foreach (MFAdmin::$_langs as $key=>$value){
                        // insert
                        $insertData = [
                            ...
                            if (in_array($file['type'], $allowedContentType)) {
                                if (in_array($imageinfo['mime'], $allowedContentType)) {
                                    ...
                                    if ($handle->uploaded) {
                                        ...
                                        $handle->image_x              = photo_gallery_width;
                                        $handle->image_ratio_y        = photo_gallery_height;
                                        ...
                                        $handle->image_x            = thumb_gallery_width;
                                        $handle->image_y            = thumb_gallery_height;
                                        $handle->dir_chmod          = 0777;
                                        $handle->process(UPLOAD_DIR . 'Image/'.$this->_menu);
                                        if ($handle->processed) {
                                            $handle->clean();
                                    ...
                                    $result = $this->db->insert('`cms_gallery_photo`', $insertData);
                                    if($result){
                                        $succeeded[] = array(
                                                'id'=>$result,
                                                'name'=>$file['name'],
    ...
    public function deletePhoto($gid=NULL, $sid=NULL){
        if(isset($gid) && isset($sid)){
            ...
            $query = "SELECT `thumb`, `photo` FROM `cms_gallery_photo` WHERE `id`=:id and `secret_id`=:secret_id LIMIT 1";
            $deleteDbWhere = "`id`={$gid} and `secret_id`='{$sid}'";
        } else {
            $selectData = [
                    'p_id'=>0
            ];
            $query = "SELECT `thumb`, `photo` FROM `cms_gallery_photo` WHERE `p_id`=:p_id and date(`date`) < CURDATE()";
        ...
        $this->db->delete('`cms_gallery_photo`', $deleteDbWhere);
    ...
    public function orderphoto(){
        ...
        if(is_array($updateRecordsArray) && !empty($updateRecordsArray)){
            ...
            foreach ($updateRecordsArray as $recordIDValue) {
                ...
                $this->db->update('`cms_gallery_photo`', $updateData, "`id`={$id}");
