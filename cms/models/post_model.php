...
class Post_Model extends Model {
    ...
    public function listItems() {
        ...
        $query = "SELECT P.`id`, P.`type`, T.`title`, T.`text`, P.`post_time`, P.`update_date`, P.`status`
                    FROM `cms_post` P
    ...
    public function deleteItem($items) {
        ...
        $result = $this->db->update('`cms_post`', $updateData, '`id` IN (' . $ids . ')');
    ...
    public function createItem() {
         ...
         * CHECK type
         */
        $data['data_type'] = isset($_POST['data_type']) ? (int) $_POST['data_type']: 0;
        ...
        if (empty($error)){
            ...
            $result = $this->db->insert('cms_post', $insertData);
            ...
            if ($result) {
                ...
                foreach (MFAdmin::$_langs as $key=>$value){
                    ...
                    $this->db->insert('cms_post_text', $insertData);
                }
    ...
    public function singleItem($id) {
         ...
         * SELECT `data`
         */
        $query = "SELECT S.`id`, S.`status`, S.`category`, S.`type`, S.`post_time`, T.`title`, T.`slug`, T.`text`, T.`lang`, 
	                  (SELECT `secret_id` FROM `cms_post_photo` WHERE p_id = S.id LIMIT 1) as  `secret_id` 
                    FROM `cms_post` S 
                    LEFT JOIN `cms_post_text` T ON T.p_id = S.id   
                    WHERE S.`id`=:id";
    ...
    public function updateItem($id) {
        ...
        foreach (MFAdmin::$_langs as $key=>$value){
            ...
            //    $result = $this->db->select($query, $selectData);
            //    if($result[0]['count']>0){
            //        $data['data_slug_'.$key] = $data['data_slug_'.$key].'-'.$result[0]['count'];
            //    }
            //}
        ...
        if(empty($error)) {
            ...
            $result = $this->db->update('`cms_post`', $updateData, "`id`={$id}");
            ...
            if (isset($result['mysql_error'])) {
                ...
                $deleted = $this->db->delete('cms_post_text', "`p_id`='{$id}'");
                if($deleted) {
                    foreach (MFAdmin::$_langs as $key=>$value){
                        // insert
                        $insertData = [
                            ...
                            if (in_array($file['type'], $allowedContentType)) {
                                if (in_array($imageinfo['mime'], $allowedContentType)) {
                                    ...
                                    if ($handle->uploaded) {
                                        ...
                                        $handle->image_x              = photo_post_width;
                                        $handle->image_ratio_y        = photo_post_height;
                                        ...
                                        $handle->image_x            = thumb_post_width;
                                        $handle->image_y            = thumb_post_height;
                                        $handle->dir_chmod          = 0777;
                                        $handle->process(UPLOAD_DIR . 'Image/'.$this->_menu);
                                        if ($handle->processed) {
                                            $handle->clean();
                                    ...
                                    $result = $this->db->insert('`cms_post_photo`', $insertData);
                                    if($result){
                                        $succeeded[] = array(
                                                'id'=>$result,
                                                'name'=>$file['name'],
    ...
    public function deletePhoto($gid=NULL, $sid=NULL){
        if(isset($gid) && isset($sid)){
            ...
            $query = "SELECT `thumb`, `photo` FROM `cms_post_photo` WHERE `id`=:id and `secret_id`=:secret_id LIMIT 1";
            $deleteDbWhere = "`id`={$gid} and `secret_id`='{$sid}'";
        } else {
            $selectData = [
                    'p_id'=>0
            ];
            $query = "SELECT `thumb`, `photo` FROM `cms_post_photo` WHERE `p_id`=:p_id and date(`date`) < CURDATE()";
        ...
        $this->db->delete('`cms_post_photo`', $deleteDbWhere);
    ...
    public function orderphoto(){
        ...
        if(is_array($updateRecordsArray) && !empty($updateRecordsArray)){
            ...
            foreach ($updateRecordsArray as $recordIDValue) {
                ...
                $this->db->update('`cms_post_photo`', $updateData, "`id`={$id}");
