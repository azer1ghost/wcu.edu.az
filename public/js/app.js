...
$(document).ready(function () {
  ...
  $("body .main-header .main-header-menu ul li").click(function () {
    if (navbarOpened == null) {
      animated_opening(this);
    } else {
      if (navbarOpened == this.id) {
  ...
  $(document).on("click", function (e) {
    //alert(navbarOpened)
    if ($(e.target).closest(".main-header-menu").length === 0) {
      if (navbarOpened) {
        navbarOpened = null;
        $(".submenu-container").css("display", "none");
      }
...
jQuery(document).ready(function ($) {
  ...
  $(".news-tabs ul li a").click(function () {
    ...
    $(
      $(".news-tabs .tabs-one-tab")[$(".news-tabs ul li a").index($(this))]
    ).addClass("active");
    $(
      $(".news-tabs .news-all-link")[$(".news-tabs ul li a").index($(this))]
    ).addClass("active");
  });
  ...
  $(".right-side-links ul li a").click(function () {
    ...
    $(this).addClass("active");
    $(
      $(".main-header-tabs")[$(".right-side-links ul li a").index($(this))]
    ).addClass("active");
  ...
  $(document).scroll(function () {
    ...
    if (
      ...
      $(".page-sidebar").offset().top + $(".page-sidebar").height()
    ) {
      $(".sidebar").css(
        "top",
        $(".page-sidebar").offset().top +
          $(".page-sidebar").height() -
          ($(".sidebar").offset().top + $(".sidebar").height())
      );
    }
  ...
  $(document).scroll(function () {
    ...
    if (
      ...
      $(".third-fixed").css(
        ...
        $(".page-sidebar").offset().top +
          $(".page-sidebar").height() -
          ($(".third-fixed").offset().top + $(".third-fixed").height() - 50)
      );
    }
  });
  ...
  function thirdRes() {
    ...
    if ($(document).width() > 1400) {
      $(".third-col").css(
        "left",
        $(".content__left").offset().left + $(".content__left").width() + 30
      );
    }
  }
  ...
  $(document).ready(function () {
    ...
    for (var i = 0; i < a.length; i++) {
      if (
        ($($(a).find("div p:last-child")[i]).height() > 42) &
        ($($(a).find("div p:last-child")[i]).height() < 65)
      ) {
        $(a[i]).height(
          $(a[i]).height() + $($(a).find("div p:last-child")[i]).height() - 42
        );
...
$(document).ready(function () {
  ...
  $(".video-block").click(function () {
    ...
    $("body .video-hidder").append(
      '<iframe width="780" height="440" src="https://www.youtube-nocookie.com/embed/' +
        $(this)
          .data("videolink")
          .substr($(this).data("videolink").indexOf("=") + 1, 11) +
  ...
  setTimeout(function () {
    for (var i = 0; i < mapsEl.length; i++) {
      ...
      $($(".map-container")[i]).height(
        $($($($(".map-container")[i]).parent()).parent()).height()
      );
      var a1 = a[0];
...
for (var i = 0; i < $(".submenu-container .submenu__list ul").length; i++) {
  ...
  if ($(el).children().length % 2 == 0) {
    ...
    $($($(el).children()[$(el).children().length / 2 - 1])).css(
      "padding-bottom",
      "0"
    );
  } else {
    // $($($(el).children()[($(el).children().length - 1) / 2])).css("border-bottom-color","#fff");
    $($($(el).children()[($(el).children().length - 1) / 2]).children()[0]).css(
      "padding-bottom",
      "0"
    );
  }
...
$(document).ready(function () {
  if (
    $(window).width() < 530 &&
    $(".content__left>div>#single_image").length > 1
  ) {
...
$(document).ready(function () {
  if ($(window).width() < 1190) {
    ...
    $(".mobile-search .search-wrap").append(
    ...
    $(".main-header-menu>ul").after(
      '<div class="mobile-close-nav" id="mobile-close-nav"></div>'
    );

    $(".mobile-close-nav").append(
...
$("#banner-modal__close").click(function () {
  $(".banner-modal").remove();
});
...
$(document).ready(function () {
  ...
  setInterval(function () {
    ...
    if (Math.floor(audioplayer.currentTime) < 59) {
      ...
      if (Math.floor(audioplayer.currentTime) < 10)
        time = "0:0" + Math.floor(audioplayer.currentTime);
    } else {
      var min = Math.floor(audioplayer.currentTime / 60);
      time = min + ":" + Math.floor(audioplayer.currentTime - min * 60);
      if (Math.floor(audioplayer.currentTime - min * 60) < 10)
        time = min + ":0" + Math.floor(audioplayer.currentTime - min * 60);
    }

    $(".player-time").html(time);

    //timeline
    var currentTimePlayer =
  ...
  $(".player-volume").click(function (e) {
    ...
    $(".player-volume div:last-child").css(
      "width",
      (relX / $(this).width()) * 100 + "%"
    );
